...
from settings.constants import DIRECTIONS, SNAKE_SIZE, SIZE, DEAD_REWARD, \
...
class World(object):
    ...
    def init_snake(self):
        ...
        if not self.custom:
            ...
            start_position = (random.randint(SNAKE_SIZE, SIZE[0] - SNAKE_SIZE),
                                   random.randint(SNAKE_SIZE, SIZE[0] - SNAKE_SIZE))
    ...
    def move_snake(self, action):
        ...
        if self.snake.alive:
            ...
            elif new_snake_head in self.snake.blocks[2:]:
                self.snake.alive = False
            #  Check if snake eats the food
            if new_snake_head in tuple(zip(*np.where(self.world == self.FOOD))):
                # Remove old food
